#!/usr/bin/env python3
"""
Pearson correlation of sRNAs feature vectors of viral and other contigs
"""
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description='Pearson correlation-based feature vector clustering analysis',
    ...
    parser.add_argument('-m', '--mean_rs', type=float, default=0.8,
    ...
    parser.add_argument('-o', '--output', type=str, default='',
                        help='Output file prefix (default: use vector dimension as prefix)')
...
def load_benchmarks(benchmarks_arg, vector_dim=None):
    """
    Load benchmark contigs
    Support file path, direct contig names, or use default reference
    vector_dim: detected vector dimension for dynamic filename
    """
    ...
    if benchmarks_arg is None:
        # Use default vsiRNA simulant reference with dynamic dimension
        default_filename = f"vsiRNA_simulant_{vector_dim}.csv"
        possible_locations = [
            default_filename,
            f"../{default_filename}",
            f"./{default_filename}"
...
def main():
    ...
    def detect_vector_dimension(numeric_cols):
        ...
        if num_cols == 13:
            ...
            print(f"Warning: Cannot recognize {num_cols}-dimensional vector, will use all numeric columns")
            return f"{num_cols}"

    # Determine vector dimension to use
    vector_dim = detect_vector_dimension(numeric_cols)

    # Loading benchmarks to use vector_dim
    target_rows = load_benchmarks(args.benchmarks, vector_dim)
    if not target_rows:
        print("Error: Cannot load benchmark contigs")
        return

    # Output name prefix
    output_prefix = args.output if args.output else vector_dim

    print("=" * 60)
    print("Pearson Correlation Clustering Analysis")
    print("=" * 60)
    ...
    print(f"Mean correlation threshold: {args.mean_rs}")
    ...
    # Feature description based on dimension type
    if vector_dim == "13":
        print("Feature description: 13-dimensional (18-30nt length distribution)")
    elif vector_dim == "17":
        print("Feature description: 17-dimensional (18-30nt length distribution + A/T/C/G base composition)")
    elif vector_dim == "52":
        print("Feature description: 52-dimensional (18-30nt × A/T/C/G base specificity)")
    elif vector_dim == "104":
        print("Feature description: 104-dimensional (18-30nt × A/T/C/G × strand specificity)")
    ...
    print("Calculating Pearson correlation for feature vectors...")
    ...
    final_rows = sorted(scoring.items(), key=lambda x: -x[1]['score'])[:args.common_contigs]

    # Apply threshold filtering
    filtered_rows = [(row, values['score'], values['p_value'])
                     for row, values in final_rows
                     if values['score'] >= args.mean_rs and values['p_value'] < args.p_value]

    print(
        f"After threshold filtering (mean correlation>={args.mean_rs}, p-value<{args.p_value}), retained {len(filtered_rows)} contigs")
    ...
    # Strict Pearson correlation-based clustering
    print("Starting strict Pearson correlation-based clustering analysis...")
    ...
    # Calculate Pearson distance matrix (1 - Pearson correlation)
    print("Calculating Pearson distance matrix...")
    ...
    # Hierarchical clustering using Pearson distance matrix
    ...
    if not args.no_plot:
        ...
        plt.tight_layout()
        heatmap_filename = f"{output_prefix}_pearson_clustering_heatmap.svg"
    ...
    try:
        ...
        df_filtered = pd.DataFrame(filtered_rows, columns=["Contig_Name", "Mean_Pearson_Correlation", "P_Value"])
        ...
        distance_df.to_csv(f'{output_prefix}_pearson_distance_matrix.csv')
        print(f"Pearson distance matrix saved as '{output_prefix}_pearson_distance_matrix.csv'")
        ...
        with open(f'{output_prefix}_analysis_summary.txt', 'w') as f:
            f.write(f"{vector_dim} Pearson Correlation Analysis Summary\n")
            f.write("=" * 50 + "\n")
            ...
            f.write(f"Mean correlation threshold: {args.mean_rs}\n")
            f.write(f"P-value threshold: {args.p_value}\n")
            f.write(f"Heatmap data: Normalized feature values (0-1 proportion)\n")
            f.write(
                f"Clustering method: Hierarchical clustering based on Pearson correlation (distance = 1 - Pearson correlation)\n")
            f.write(f"Data normalization: Each feature value divided by row total (proportion normalization)\n")

            if vector_dim == "13":
                f.write(f"Feature dimension: 13-dimensional (18-30nt length distribution)\n")
            elif vector_dim == "17":
                f.write(f"Feature dimension: 17-dimensional (18-30nt length distribution + A/T/C/G base composition)\n")
            elif vector_dim == "52":
                f.write(f"Feature dimension: 52-dimensional (18-30nt × A/T/C/G base specificity)\n")
            elif vector_dim == "104":
                f.write(f"Feature description: 104-dimensional (18-30nt × A/T/C/G × strand specificity)\n")
        ...
        print(f"  - {output_prefix}_pearson_distance_matrix.csv: Pearson distance matrix")
        print(f"  - {output_prefix}_clustering_order.csv: Clustering order")
        print(f"  - {output_prefix}_analysis_summary.txt: Analysis summary")
        if not args.no_plot:
            print(f"  - {output_prefix}_pearson_clustering_heatmap.svg: Clustering heatmap")

    except Exception as e:
        print(f"Error saving files: {e}")

    print("\n" + "=" * 60)
    print(f"{vector_dim} Pearson Clustering Analysis Completion Summary:")
    print(f"Input file: {args.input}")
