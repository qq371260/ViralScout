...
def parse_arguments():
    ...
    parser.add_argument('-o', '--output', default='sRNA_original_feature_vectors.csv',
...
def generate_column_headers(mode, min_length, max_length):
    """Generate column headers for a given mode and length range"""
...
def adapt_external_vectors_to_length_range(external_vectors_df, mode, target_min_length, target_max_length):
    """
    Adapt external vectors to match target length range by both truncating and extending
    """
    print(f"Adapting external vectors to target length range: {target_min_length}-{target_max_length} nt")

    # Get the column names from the external vectors (excluding 'Reference')
    external_columns = external_vectors_df.columns.tolist()[1:]

    # Generate expected column names for the default length range (18-30)
    default_columns = generate_column_headers(mode, 18, 30)[1:]

    # Verify that external vectors match the default structure
    if external_columns != default_columns:
        raise ValueError(
            f"External vector columns don't match expected default structure for mode {mode}.\n"
            f"Expected: {default_columns[:5]}... (total {len(default_columns)} columns)\n"
            f"Got: {external_columns[:5]}... (total {len(external_columns)} columns)"
        )

    # If target range is the same as default, no adaptation needed
    if target_min_length == 18 and target_max_length == 30:
        print("Length ranges match, no adaptation needed")
        return external_vectors_df

    # Generate target column structure
    target_columns = generate_column_headers(mode, target_min_length, target_max_length)
    ...
    for idx, row in external_vectors_df.iterrows():
        ref_name = row.iloc[0]
        adapted_row = [ref_name]

        # For each target column (excluding Reference), find corresponding value or use 0
        for target_col in target_columns[1:]:
            if target_col in external_columns:
                # Column exists in external vectors, use the value
                col_idx = external_columns.index(target_col) + 1  # +1 for Reference column
                adapted_row.append(row.iloc[col_idx])
            else:
                # Column doesn't exist in external vectors, use 0
                adapted_row.append(0)

        adapted_rows.append(adapted_row)

    # Create new DataFrame with adapted data
    adapted_df = pd.DataFrame(adapted_rows, columns=target_columns)

    print(f"Adapted external vectors from {len(external_columns)} to {len(adapted_df.columns) - 1} columns")
    return adapted_df
    ...
    if benchmarks_file is None:
        ...
        for ref_file in possible_locations:
            if os.path.isfile(ref_file):
                ...
                # Adapt external vectors to match target length range
                if (target_min_length != 18 or target_max_length != 30):
                    ref_df = adapt_external_vectors_to_length_range(
                        ref_df, mode, target_min_length, target_max_length
                    )
...
def main():
    ...
    with open(args.output, 'w', newline='') as csvfile:
        ...
        headers = generate_column_headers(args.mode, args.min_length, args.max_length)
